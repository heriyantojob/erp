...
const latestJournalBefore = entriesBefore.reduce((max, entry) => Math.max(max, entry.idx), -1);
...
if (needsSnapshotBaseline) {
  ...
  console.info('Creating a safe schema baseline snapshot. Duplicate SQL will not be executed.');
}
...
if (needsSnapshotBaseline) {
  ...
  const newEntries = (journalAfter.entries ?? []).filter((entry) => entry.idx > latestJournalBefore);

  if (newEntries.length === 0) {
    console.info('No baseline migration file was generated. Nothing else to do.');
    process.exit(0);
  }
  ...
  for (const entry of newEntries) {
    ...
    console.info(`Converted ${entry.tag}.sql into a safe no-op baseline migration.`);
  }

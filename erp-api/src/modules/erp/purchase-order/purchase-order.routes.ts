...
router.post(
  "/process/purchase-orders",
  requireAnyPermission("purchase_order.manage"),
  async (req, res) => {
    try {
      ...
      const row = await db.transaction(async (tx) => {
        ...
        if (!supplier)
          throw new Error("Supplier could not be resolved or created.");
...
router.post(
  "/process/purchase-orders/:id/status",
  requireAnyPermission("transaction.approve"),
  async (req, res) => {
    try {
      const x = poStatus.parse(req.body);
      const row = (
        await db
          ...
          .set({
            status: x.status,
            approvedByUserId:
              x.status === "approved" ? res.locals.user?.id : null,
            approvedAt: x.status === "approved" ? new Date() : null,
            updatedAt: new Date(),
          })

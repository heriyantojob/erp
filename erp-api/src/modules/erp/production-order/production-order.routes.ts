...
router.get("/process/production-orders", requireAnyPermission("production_order.view","production_order.create","material_issue.create"), async (_req,res)=>res.json(await db.select().from(productionOrders).where(isNull(productionOrders.deletedAt)).orderBy(desc(productionOrders.createdAt))));
router.post("/process/production-orders", requireAnyPermission("production_order.create"), async (req, res) => {
  try {
    ...
    const st = await db.transaction(async (tx) => {
      ...
      await audit(tx, res, "production_order_create", "production_order", row.id, row);
      return row;
    });
...
router.post("/process/production-orders/:id/start", requireAnyPermission("production.manage","production_order.create"), async (req,res)=>{try{const row=(await db.update(productionOrders).set({status:"in_progress",updatedAt:new Date()}).where(eq(productionOrders.id,String(req.params.id))).returning())[0];if(!row)return res.status(404).json({message:"Production order not found"});await audit(db,res,"production_start","production_order",row.id,row);res.json(row);}catch(e){sendApiError(req,res,e);} });
router.post("/process/production-orders/:id/complete", requireAnyPermission("production.manage","production_order.create"), async (req,res)=>{try{const row=(await db.update(productionOrders).set({status:"completed",updatedAt:new Date()}).where(eq(productionOrders.id,String(req.params.id))).returning())[0];if(!row)return res.status(404).json({message:"Production order not found"});await audit(db,res,"production_complete","production_order",row.id,row);res.json(row);}catch(e){sendApiError(req,res,e);} });

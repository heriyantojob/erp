...
export async function ensureSupplierFromBusinessPartner(db: any, supplierCode: string) {
...
export async function findBatch(db: any, materialCode: string, batchNumber: string) {
  return (await db
    .select({
      id: batchLots.id,

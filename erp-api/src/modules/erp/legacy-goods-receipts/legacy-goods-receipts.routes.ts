...
router.get(
  "/goods-receipts",
  requireAnyPermission("goods_receipt.view"),
  async (_req, res) =>
    res.json(
      await db
        .select()
        .from(goodsReceipts)
        .where(isNull(goodsReceipts.deletedAt))
        .orderBy(asc(goodsReceipts.receivedAt), asc(goodsReceipts.receiptCode)),
    ),
);
router.post(
  "/goods-receipts",
  ...
  async (req, res) => {
    try {
      const data = receiptInput.parse(req.body);
      const row = (
        await db
      ...
      if (!row) throw new Error("Goods receipt was not created");
      await audit(
        db,
        res,
        "create",
        "legacy_goods_receipt",
        row.id,
        undefined,
        row,
      );
...
router.put(
  "/goods-receipts/:id",
  requireAnyPermission("transaction.review"),
  async (req, res) => {
    try {
      ...
      const before = (
        await db
          ...
          .from(goodsReceipts)
          .where(
            and(eq(goodsReceipts.id, key), isNull(goodsReceipts.deletedAt)),
          )
      )[0];
      const row = (
        await db
      ...
      return row
        ? res.json(row)
        : res.status(404).json({ message: "Goods receipt tidak ditemukan" });
    } catch (error) {
      return sendApiError(req, res, error);
    }
  },
...
router.delete(
  ...
  async (_req, res) => {
    return res.status(405).json({
      message:
        "Posted transactions cannot be deleted. Create a reversal with a recorded reason.",
    });
  },
);

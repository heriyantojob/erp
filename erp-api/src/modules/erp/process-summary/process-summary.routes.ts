...
router.get("/process/summary", async (_req, res) => {
  const [prs, pos, qas, pros, fgs, sos, ds] = await Promise.all([
    ...
    db.select().from(purchaseOrders).where(isNull(purchaseOrders.deletedAt)),
    db.select().from(qualityInspections),
    db
      .select()
      .from(productionOrders)
      .where(isNull(productionOrders.deletedAt)),
    db.select().from(finishedGoodsReceipts),
    db.select().from(salesOrders).where(isNull(salesOrders.deletedAt)),
    db.select().from(deliveryHeaders).where(isNull(deliveryHeaders.deletedAt)),
  ]);
  res.json({
    purchaseRequisition: prs.length,
    purchaseOrder: pos.length,
    incomingQuality: qas.filter((x) => x.status === "pending").length,
    productionOrder: pros.length,
    manufacturing: pros.filter((x) => x.status === "in_progress").length,
    finishedGoodsReceipt: fgs.length,
    salesOrder: sos.length,
    customerDelivery: ds.length,
  });
});

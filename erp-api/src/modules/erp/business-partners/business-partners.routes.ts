  ...
  async (req, res) => {
    try {
      ...
      if (!supplier)
        return res
          .status(404)
          .json({ message: `Supplier "${codeValue}" not found.` });
      return res.json({
        value: supplier.code,
        label: `${supplier.code} - ${supplier.name}`,
        code: supplier.code,
...
router.get(
  "/business-partners",
  requireAnyPermission("business_partners.view"),
  async (_req, res) =>
    res.json(
      await db
        .select()
        .from(businessPartners)
        .where(isNull(businessPartners.deletedAt))
        .orderBy(asc(businessPartners.code)),
    ),
);
router.post(
  "/business-partners",
  requireAnyPermission("business_partners.manage"),
  async (req, res) => {
    try {
      const data = partnerInput.parse(req.body);
      const row = (
        await db.insert(businessPartners).values(data).returning()
      )[0];
      if (!row) throw new Error("Business partner was not created");
      await audit(
        db,
        res,
        "create",
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const data = partnerInput.omit({ code: true }).parse(req.body);
      const before = (
        await db
          .select()
          .from(businessPartners)
      ...
      return row
        ? res.json(row)
        : res.status(404).json({ message: "Mitra bisnis tidak ditemukan" });
    } catch (error) {
      return sendApiError(req, res, error);
    }
  },
  ...
  async (req, res) => {
    try {
      ...
      if (!before)
        return res
          .status(404)
          .json({ message: "Mitra bisnis tidak ditemukan" });
      const row = (
        await db
          .update(businessPartners)
          .set({

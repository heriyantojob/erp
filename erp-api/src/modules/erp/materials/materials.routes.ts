...
router.get(
  "/materials",
  requireAnyPermission("materials.view"),
  async (_req, res) =>
    res.json(
      await db
        .select()
...
router.post(
  "/materials",
  requireAnyPermission("materials.manage"),
  async (req, res) => {
    try {
      const data = materialInput.parse(req.body);
      const row = await db.transaction(async (tx) => {
        const created = (
          await tx.insert(materials).values(data).returning()
        )[0];
      ...
      const row = await db.transaction(async (tx) => {
        const before = (
          await tx
            .select()
            .from(materials)
            .where(and(eq(materials.code, key), isNull(materials.deletedAt)))
        )[0];

...
router.get("/process/sales-orders", requireAnyPermission("sales_order.manage","stock.view"), async (_req,res)=>res.json(await db.select().from(salesOrders).where(isNull(salesOrders.deletedAt)).orderBy(desc(salesOrders.createdAt))));
router.post("/process/sales-orders", requireAnyPermission("sales_order.manage"), async (req, res) => {
  try {
    const x = salesInput.parse(req.body);
    const row = await db.transaction(async (tx) => {
      await tx.execute(sql`SELECT id FROM stock_balances WHERE id=${x.stockBalanceId} FOR UPDATE`);
      const bal = (await tx.select().from(stockBalances).where(eq(stockBalances.id, x.stockBalanceId)))[0];
      if (!bal) throw new Error("Stock balance not found");
      const batch = (await tx.select().from(batchLots).where(eq(batchLots.id, bal.batchLotId)))[0];
      if (batch?.qualityStatus !== "released") throw new Error("Only released stock can be reserved for sales");
      const active = await tx.select({ q: stockReservations.quantity }).from(stockReservations).where(and(eq(stockReservations.stockBalanceId, bal.id), eq(stockReservations.status, "active")));
      ...
      if (onhand - reserved < requested) throw new Error(`Insufficient available stock. Available ${dec(onhand-reserved)} ${bal.unit}.`);

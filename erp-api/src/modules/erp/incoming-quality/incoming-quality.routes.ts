...
router.get("/process/incoming-quality", requireAnyPermission("quality.manage","goods_receipt.view"), async (_req,res)=>{ const rows=await db.select({id:qualityInspections.id,receiptId:qualityInspections.receiptId,receiptNumber:goodsReceiptHeaders.receiptNumber,batchLotId:qualityInspections.batchLotId,batchNumber:batchLots.batchNumber,materialCode:batchLots.materialCode,quantity:goodsReceiptLines.quantity,unit:goodsReceiptLines.unit,status:qualityInspections.status,result:qualityInspections.result,inspectedAt:qualityInspections.inspectedAt}).from(qualityInspections).innerJoin(goodsReceiptHeaders,eq(qualityInspections.receiptId,goodsReceiptHeaders.id)).innerJoin(batchLots,eq(qualityInspections.batchLotId,batchLots.id)).innerJoin(goodsReceiptLines,and(eq(goodsReceiptLines.receiptId,qualityInspections.receiptId),eq(goodsReceiptLines.batchLotId,qualityInspections.batchLotId))).orderBy(desc(qualityInspections.createdAt)); res.json(rows); });
router.post("/process/incoming-quality/:id/inspect", requireAnyPermission("quality.manage", "transaction.approve"), async (req, res) => {
  try {
    const x = qaInput.parse(req.body);
    const result = await db.transaction(async (tx) => {
      const before = (await tx.select().from(qualityInspections).where(eq(qualityInspections.id, String(req.params.id))))[0];
      if (!before) return null;
      const row = (await tx.update(qualityInspections).set({
        status: x.status,
        result: x.result,
        inspectedByUserId: res.locals.user?.id,
        inspectedAt: new Date(),
        updatedAt: new Date(),
      }).where(eq(qualityInspections.id, before.id)).returning())[0];
      if (!row) throw new Error("Quality inspection update failed.");
      await tx.update(batchLots).set({ qualityStatus: x.status, updatedAt: new Date() }).where(eq(batchLots.id, before.batchLotId));
      await tx.update(goodsReceiptLines).set({ qualityStatus: x.status }).where(and(eq(goodsReceiptLines.receiptId, before.receiptId), eq(goodsReceiptLines.batchLotId, before.batchLotId)));
      await audit(tx, res, "incoming_quality_inspect", "quality_inspection", row.id, row);
      return row;
    });
    if (!result) return res.status(404).json({ message: "Quality inspection not found" });

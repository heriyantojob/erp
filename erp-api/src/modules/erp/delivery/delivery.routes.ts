...
router.get("/process/deliveries", requireAnyPermission("delivery.create","stock.view"), async (_req,res)=>res.json(await db.select({id:deliveryHeaders.id,deliveryNumber:deliveryHeaders.deliveryNumber,deliveredAt:deliveryHeaders.deliveredAt,status:deliveryHeaders.status,salesOrderNumber:deliveryHeaders.salesOrderNumber,materialCode:deliveryLines.materialCode,quantity:deliveryLines.quantity,unit:deliveryLines.unit}).from(deliveryHeaders).leftJoin(deliveryLines,eq(deliveryLines.deliveryId,deliveryHeaders.id)).where(isNull(deliveryHeaders.deletedAt)).orderBy(desc(deliveryHeaders.createdAt))));
router.post("/process/deliveries", requireAnyPermission("delivery.create"), async (req, res) => {
  try {
    const x = deliveryInput.parse(req.body);
    const row = await db.transaction(async (tx) => {
      const so = (await tx.select().from(salesOrders).where(eq(salesOrders.id, x.salesOrderId)))[0];
      if (!so || !so.stockBalanceId || !so.reservationId) throw new Error("Confirmed sales order with reservation not found");
      if (so.status === "delivered") throw new Error("Sales order is already delivered");
      await tx.execute(sql`SELECT id FROM stock_balances WHERE id=${so.stockBalanceId} FOR UPDATE`);
      const bal = (await tx.select().from(stockBalances).where(eq(stockBalances.id, so.stockBalanceId)))[0];
      if (!bal) throw new Error("Stock balance not found");
      const customerCode = so.customerCode ?? "WALK-IN";
      const existingCustomer = (await tx.select().from(customers).where(eq(customers.code, customerCode)))[0];
      if (!existingCustomer) await tx.insert(customers).values({ code: customerCode, name: customerCode });
      ...
      const header = (await tx.insert(deliveryHeaders).values({
      ...
      await tx.insert(deliveryLines).values({ deliveryId: header.id, lineNumber: 1, materialCode: so.materialCode, batchLotId: bal.batchLotId, locationId: bal.locationId, quantity: String(so.quantity), unit: so.unit });
      await tx.update(stockBalances).set({ quantityOnHand: dec(on-q), updatedAt: new Date() }).where(eq(stockBalances.id, bal.id));
      await tx.update(stockReservations).set({ status: "consumed", releasedAt: new Date(), releasedByUserId: res.locals.user?.id, releaseReason: "Consumed by customer delivery", updatedAt: new Date() }).where(eq(stockReservations.id, so.reservationId));
      await tx.update(salesOrders).set({ status: "delivered", updatedAt: new Date() }).where(eq(salesOrders.id, so.id));
      await tx.insert(stockTransactions).values({ transactionNumber: `DL-${crypto.randomUUID()}`, transactionType: "customer_delivery", materialCode: so.materialCode, batchLotId: bal.batchLotId, warehouseId: bal.warehouseId, locationId: bal.locationId, quantityIn: "0", quantityOut: String(so.quantity), unit: so.unit, referenceType: "customer_delivery", referenceId: header.id, createdByUserId: res.locals.user?.id });
      await audit(tx, res, "customer_delivery_post", "delivery", header.id, header);
      return header;
    });

...
router.get("/process/purchase-requisitions", requireAnyPermission("purchase_requisition.manage","transaction.approve"), async (_req,res)=>res.json(await db.select().from(purchaseRequisitions).where(isNull(purchaseRequisitions.deletedAt)).orderBy(desc(purchaseRequisitions.createdAt))));
router.post("/process/purchase-requisitions", requireAnyPermission("purchase_requisition.manage"), async (req, res) => {
  try {
    const x = prInput.parse(req.body);
    const row = (await db.insert(purchaseRequisitions).values({
      ...x,
    ...
    await audit(db, res, "purchase_requisition_submit", "purchase_requisition", row.id, row);
    return res.status(201).json(row);
  } catch (e) {
    return sendApiError(req, res, e);
  }
});
router.post("/process/purchase-requisitions/:id/approve", requireAnyPermission("transaction.approve"), async (req,res)=>{ try { const row=(await db.update(purchaseRequisitions).set({status:"approved",approvedByUserId:res.locals.user?.id,approvedAt:new Date(),updatedAt:new Date()}).where(eq(purchaseRequisitions.id,String(req.params.id))).returning())[0]; if(!row)return res.status(404).json({message:"Purchase requisition not found"}); await audit(db,res,"purchase_requisition_approve","purchase_requisition",row.id,row); res.json(row); }catch(e){sendApiError(req,res,e);} });

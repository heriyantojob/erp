...
export function error(err: unknown, req: Request, res: Response, _next: NextFunction) {
  const locale = getApiLocale(req);
  const candidate = err as Partial<CustomError> & { statusCode?: number; code?: string; message?: string };
  ...
  return res.status(status).json({
    locale,
    code: candidate?.code || (status >= 500 ? "INTERNAL_SERVER_ERROR" : "REQUEST_ERROR"),
    message,

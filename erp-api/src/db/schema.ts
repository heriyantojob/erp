...
import { boolean, date, jsonb, numeric, pgTable, smallint, text, timestamp, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
...
export const locations = pgTable("locations", {
  ...
  name: varchar("name", { length: 255 }).notNull(),
  locationType: varchar("location_type", { length: 50 }).notNull().default("storage"),
  active: boolean("active").notNull().default(true),
  createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("locations_warehouse_code_unique").on(table.warehouseId, table.code)]);
...
export const batchLots = pgTable("batch_lots", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  materialCode: varchar("material_code", { length: 50 }).notNull().references(() => materials.code),
  ...
  receivedAt: date("received_at", { mode: "string" }).notNull().default(sql`CURRENT_DATE`),
  manufacturedAt: date("manufactured_at", { mode: "string" }),
  expiryDate: date("expiry_date", { mode: "string" }),
  qualityStatus: varchar("quality_status", { length: 30 }).notNull().default("quarantine"),
  traceParentBatchId: uuid("trace_parent_batch_id").references((): any => batchLots.id),
  createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("batch_lots_material_batch_unique").on(table.materialCode, table.batchNumber)]);
...
export const goodsReceiptHeaders = pgTable("goods_receipt_headers", {
  ...
  supplierCode: varchar("supplier_code", { length: 50 }).references(() => suppliers.code),
  warehouseId: uuid("warehouse_id").notNull().references(() => warehouses.id),
...
export const goodsReceiptLines = pgTable("goods_receipt_lines", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  receiptId: uuid("receipt_id").notNull().references(() => goodsReceiptHeaders.id, { onDelete: "cascade" }),
  lineNumber: smallint("line_number").notNull(),
  materialCode: varchar("material_code", { length: 50 }).notNull().references(() => materials.code),
  batchLotId: uuid("batch_lot_id").notNull().references(() => batchLots.id),
  locationId: uuid("location_id").references(() => locations.id),
  quantity: numeric("quantity", { precision: 14, scale: 3 }).notNull(),
  unit: varchar("unit", { length: 30 }).notNull(),
  qualityStatus: varchar("quality_status", { length: 30 }).notNull().default("quarantine"),
}, (table) => [uniqueIndex("goods_receipt_lines_receipt_line_unique").on(table.receiptId, table.lineNumber)]);
...
export const qualityInspections = pgTable("quality_inspections", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  receiptId: uuid("receipt_id").notNull().references(() => goodsReceiptHeaders.id),
  batchLotId: uuid("batch_lot_id").notNull().references(() => batchLots.id),
  ...
  createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("quality_inspections_receipt_batch_unique").on(table.receiptId, table.batchLotId)]);


export const productionOrders = pgTable("production_orders", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  orderNumber: varchar("order_number", { length: 50 }).notNull().unique(),
  finishedMaterialCode: varchar("finished_material_code", { length: 50 }).notNull().references(() => materials.code),
  plannedQuantity: numeric("planned_quantity", { precision: 14, scale: 3 }).notNull(),
  unit: varchar("unit", { length: 30 }).notNull(),
  warehouseId: uuid("warehouse_id").notNull().references(() => warehouses.id),
  status: varchar("status", { length: 30 }).notNull().default("draft"),
...
}, (table) => [uniqueIndex("production_material_issues_order_line_unique").on(table.productionOrderId, table.lineNumber)]);
...
export const deliveryHeaders = pgTable("delivery_headers", {
  ...
  customerCode: varchar("customer_code", { length: 50 }).notNull().references(() => customers.code),
  warehouseId: uuid("warehouse_id").notNull().references(() => warehouses.id),
  status: varchar("status", { length: 30 }).notNull().default("draft"),
...
export const deliveryLines = pgTable("delivery_lines", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  deliveryId: uuid("delivery_id").notNull().references(() => deliveryHeaders.id, { onDelete: "cascade" }),
  lineNumber: smallint("line_number").notNull(),
  materialCode: varchar("material_code", { length: 50 }).notNull().references(() => materials.code),
  batchLotId: uuid("batch_lot_id").notNull().references(() => batchLots.id),
  locationId: uuid("location_id").references(() => locations.id),
  quantity: numeric("quantity", { precision: 14, scale: 3 }).notNull(),
  unit: varchar("unit", { length: 30 }).notNull(),
}, (table) => [uniqueIndex("delivery_lines_delivery_line_unique").on(table.deliveryId, table.lineNumber)]);

export const stockTransactions = pgTable("stock_transactions", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  transactionNumber: varchar("transaction_number", { length: 80 }).notNull().unique(),
  transactionType: varchar("transaction_type", { length: 50 }).notNull(),
  transactionAt: timestamp("transaction_at", { mode: "date" }).notNull().defaultNow(),
  materialCode: varchar("material_code", { length: 50 }).notNull().references(() => materials.code),
  batchLotId: uuid("batch_lot_id").references(() => batchLots.id),
  warehouseId: uuid("warehouse_id").notNull().references(() => warehouses.id),
  locationId: uuid("location_id").references(() => locations.id),
  quantityIn: numeric("quantity_in", { precision: 14, scale: 3 }).notNull().default("0"),
  quantityOut: numeric("quantity_out", { precision: 14, scale: 3 }).notNull().default("0"),
  unit: varchar("unit", { length: 30 }).notNull(),
...
export const stockBalances = pgTable("stock_balances", {
  ...
  unit: varchar("unit", { length: 30 }).notNull(),
  updatedAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("stock_balances_balance_key_unique").on(table.materialCode, table.batchLotId, table.warehouseId, table.locationId)]);

export const stockReservations = pgTable("stock_reservations", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  reservationNumber: varchar("reservation_number", { length: 80 }).notNull().unique(),
  stockBalanceId: uuid("stock_balance_id").notNull().references(() => stockBalances.id),
  quantity: numeric("quantity", { precision: 14, scale: 3 }).notNull(),
  unit: varchar("unit", { length: 30 }).notNull(),
  referenceType: varchar("reference_type", { length: 50 }).notNull().default("manual"),
...
export const schema = { user, session, account, verification, files, fileAttachments, materials, businessPartners, suppliers, customers, warehouses, locations, roles, permissions, userRoles, rolePermissions, batchLots, goodsReceipts, billOfMaterials, goodsReceiptHeaders, goodsReceiptLines, purchaseRequisitions, purchaseOrders, qualityInspections, productionOrders, productionMaterialIssues, finishedGoodsReceipts, deliveryHeaders, deliveryLines, stockTransactions, stockBalances, stockReservations, salesOrders, auditLogs };

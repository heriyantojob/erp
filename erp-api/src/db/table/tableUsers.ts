import { bigint, boolean, integer, jsonb, pgTable, smallint, text, timestamp, uuid, varchar } from "drizzle-orm/pg-core";
...
export const tableUserFiles = pgTable(prefix + "user_files", {
  ...
  project_type: smallint("project_type").default(0).notNull(),
    //project   | file 
  moduleType: varchar("module_type", { length: 30 })
  .notNull()
  .default("file"),
...
export const tableUserFileAttachments = pgTable(prefix + "user_file_attachments", {
...
export const tableUserFolders = pgTable(prefix + "user_folders", {
  ...
  id: varchar("id", { length: 36 }).primaryKey().default(sql`gen_random_uuid()`),
  parentFolderId: varchar("parent_folder_id", { length: 256 }).notNull().unique(),

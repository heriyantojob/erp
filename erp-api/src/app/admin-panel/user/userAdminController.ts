...
export async function listUser(
  ...
  const whereSQL = [isNull(user.deletedAt)];
  // if (qParams ) {
...
export async function updateUser(
  ...
  try {
    let body = req.body;
    let updateUserData = await updateUserField(
      body,
      userAuth?.user.accessLevel,
    );
    const note = await db
...
export async function uploadUser(req: Request, res: Response) {
  let userAuth = res.locals;
  const file = req?.file;

  ...
  try {
    ...
    if (slugLink) {
      ...
      if (count > 0) {
        return res
          .status(400)
          .json({ message: "The link has been used already." });
      }
    }
    ...
    await s3UploadFile({
      fileBuffer: projectBuffer,
      fileName: projectPath,
      contentType: projectMime,
    });
    const headProject = await s3HeadObjectCommand({ objectKey: projectPath });
    const fileSizeProject = headProject?.ContentLength
      ? Number(headProject.ContentLength)
      : 0;
    ...
    const base64 = String(body?.preview || "").replace(
      /^data:image\/\w+;base64,/,
      "",
    );
    const base64Buf = Buffer.from(base64, "base64");
    const previewBuffer = await sharp(base64Buf)
    ...
    const fileSizePreview = headPreview?.ContentLength
      ? Number(headPreview.ContentLength)
      : 0;

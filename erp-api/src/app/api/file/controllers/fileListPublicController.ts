...
export async function listFilePublic(req: Request, res: Response, next: NextFunction) {
  const pageParam = parseInt(String(req.query.page || `${DEFAULT_PAGE}`));
  const perPageParam = parseInt(String(req.query.per_page || `${DEFAULT_PER_PAGE}`));
  ...
  try {
    const { items, total, totalPages } = await listPublicFilesService({ page, perPage, q: qParam as string, type: typeParam as string , moduleType: moduleTypeParams as string });
    if (!items?.length) {
...
export async function listFileUser(req: Request, res: Response, next: NextFunction) {  // Changed to listFileUser
...
export async function listFileSitemapController(req: Request, res: Response) {
  ...
  const perPageParam = parseInt(String(req.query.per_page || req.query.perPage || '5000'));
  const page = Number.isFinite(pageParam) && pageParam > 0 ? pageParam : 1;
  const perPage = Number.isFinite(perPageParam) && perPageParam > 0 ? perPageParam : 5000;
...
export interface TableFileWithUserType {  // Updated from TableTemplateWithUserType to TableFileWithUserType

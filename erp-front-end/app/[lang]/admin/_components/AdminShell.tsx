...
export default function AdminShell({
  ...
  const activeProcessIndex = processFlow.findIndex(
    ([, suffix]) => pathname === `${base}${suffix}`,
  );
  const { data: session, isPending } = useSession();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const belongsTo = (items: readonly NavItem[]) =>
    items.some(([, suffix]) => pathname === `${base}${suffix}`);
  ...
  const group = (
    ...
    return (
      <section
        className="border-t border-slate-800 pt-2 first:border-t-0"
        key={name}
      >
        <button
          type="button"
        ...
        {open && (
          <div className="mt-1 grid gap-1 pl-1">
            {items.map((item) => navLink(item, closeAfterNavigate))}
          </div>
        )}
      </section>
    );
      ...
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-50 md:hidden">
            <button
              aria-label={dictionary.admin.closeMenu}
            ...
            <aside className="relative flex h-full w-72 flex-col bg-slate-950 p-5 text-slate-100 shadow-2xl">
              <button
                aria-label={dictionary.admin.closeMenu}
                onClick={() => setMobileMenuOpen(false)}
                className="ml-auto rounded border border-slate-600 px-3 py-1 text-lg"
              >
                ×
              </button>
              {navigation(true)}
        ...
        <div className="min-w-0 flex-1 p-4 md:p-8">
          <header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-300 pb-5">
            ...
            <button
              ...
              className="rounded border border-slate-300 bg-white px-3 py-1.5 text-xl md:hidden"
            >
              ☰
            </button>
          </header>
            ...
            <div className="mt-5 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between gap-3">
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">
                  {dictionary.admin.sections.process}
                </p>
                <p className="text-xs text-slate-500">
                  {activeProcessIndex + 1} / {processFlow.length}
                </p>
              </div>
              <div className="flex gap-2 overflow-x-auto pb-1">
                {processFlow.map(([key, suffix], index) => (
                  ...
                  >
                    ...
                    {dictionary.admin.nav[key].replace(/^\d+\s*·\s*/, "")}
                  </Link>
                ))}
              </div>
            </div>
